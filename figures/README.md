...
`figures/output/` is not tracked in this repository. Running any
`fig*.py` / `suppfig*.py` script writes the regenerated PNG/PDF/SVG there
(see the commands below). The hand-drawn Fig. 1 schematic and the manually
composited manuscript figures are distributed with the manuscript submission
package, not here.
...
- Fig. 1 is the manuscript's hand-drawn workflow schematic (no code
  generator); the image file is distributed with the manuscript submission
  package rather than this repository.
- `Figure2_corrected.png`, `Figure5_corrected.png`, and
  `Figure8_corrected.png` are the exact manuscript composites, also
  distributed with the manuscript submission package. Fig. 2 and
  Fig. 5 were assembled for the manuscript at a 4:3 aspect ratio from the
  script-generated panels; the scripts
...
- The current simulation engine caps `edge_fraction` at 1.0 (`"auto"`). Some
  ...
  archived manuscript composites.
