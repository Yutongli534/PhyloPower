#!/usr/bin/env python3
"""Figure 2/3: statistical fidelity audit for synthetic sample pools.
...
Run with the QIIME/Gemelli environment, for example:
  /opt/miniconda3/envs/qiime2-metagenome-2024.10/bin/python fig2_fidelity_audit.py \
      --out fig2_fidelity_audit
"""
...
from fig2 import _distance_metrics, _eigenspectrum, _sample_syn_columns, _within_between  # noqa: E402
...
def main(argv=None) -> None:
    ...
    if args.skip_optimization:
        optimization = {
            "protein": {"params": {"edge_fraction": 1.25, "residual_mode": "template", "marginal_strength": "auto", "eb_k": "auto", "jitter_sd": 0.3}},
    ...
    fig.savefig(args.out / "fig2.png", dpi=320, bbox_inches="tight")
    fig.savefig(args.out / "fig2.pdf", bbox_inches="tight")
    plt.close(fig)
    ...
    _write_tables(args.out, baseline, validation, optimization, sample_stats, distance_tail, literature_tests, literature_summary)
    _plot_protein_structure(args.out, protein_real, protein_rep)
    print(f"[fig2-audit] done -> {args.out / 'fig2.png'}", flush=True)
